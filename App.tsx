...
import { UploadMode, TagStyle } from './types';
import { generateTags } from './services/geminiService';
import { UploadIcon, CopyIcon, DownloadIcon, CheckIcon, LoaderIcon } from './components/icons';

const App: React.FC = () => {
  const [uploadMode, setUploadMode] = useState<UploadMode>(UploadMode.Single);
  const [tagStyle, setTagStyle] = useState<TagStyle>(TagStyle.Flux);
  ...
  const handleBatchUpload = async () => {
    ...
    try {
      ...
      // FIX: Explicitly type 'file' as 'any' to resolve properties 'dir' and 'name' on an unknown type.
      const imageFiles = Object.values(loadedZip.files).filter(
        (file: any) => !file.dir && /\.(jpe?g|png|webp)$/i.test(file.name)
      );
      ...
      // FIX: Explicitly type 'imageFile' as 'any' to resolve properties like 'async' and 'name' on an unknown type.
      const tagGenerationPromises = imageFiles.map(async (imageFile: any) => {
        try {
          ...
          <div className="flex flex-col space-y-6">
            ...
            <div>
              ...
              <div className="flex space-x-2 bg-gray-900 p-1 rounded-lg">
                <button onClick={() => { setUploadMode(UploadMode.Single); resetState(); }} className={`w-full py-2 px-4 rounded-md text-sm font-medium transition-colors ${uploadMode === UploadMode.Single ? 'bg-cyan-500 text-white shadow' : 'hover:bg-gray-700'}`}>Single Image</button>
                <button onClick={() => { setUploadMode(UploadMode.Batch); resetState(); }} className={`w-full py-2 px-4 rounded-md text-sm font-medium transition-colors ${uploadMode === UploadMode.Batch ? 'bg-cyan-500 text-white shadow' : 'hover:bg-gray-700'}`}>Batch (ZIP)</button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div onClick={() => setTagStyle(TagStyle.Flux)} className={`p-4 rounded-lg cursor-pointer transition-all border-2 ${tagStyle === TagStyle.Flux ? 'border-purple-500 bg-purple-500/10' : 'border-gray-700 hover:border-purple-400 bg-gray-900'}`}>
                  <h4 className="font-bold">✨ Flux Style</h4>
                  <p className="text-sm text-gray-400 mt-1">Creative, human-like captions.</p>
                </div>
                <div onClick={() => setTagStyle(TagStyle.Illustrious)} className={`p-4 rounded-lg cursor-pointer transition-all border-2 ${tagStyle === TagStyle.Illustrious ? 'border-purple-500 bg-purple-500/10' : 'border-gray-700 hover:border-purple-400 bg-gray-900'}`}>
            ...
            <div>
              ...
              <div className="relative border-2 border-dashed border-gray-600 rounded-lg p-6 text-center hover:border-cyan-400 transition-colors">
                ...
                <p className="text-xs text-gray-500 mt-1">{uploadMode === UploadMode.Single ? 'PNG, JPG, WEBP' : 'ZIP file with images'}</p>
                <input
                  ref={fileInputRef}
                  type="file"
              ...
              {selectedFile && <p className="text-sm text-gray-400 mt-2 truncate">Selected: {selectedFile.name}</p>}
            </div>
             <button
          ...
          <div className="bg-gray-900 p-6 rounded-lg flex flex-col">
            ...
            {!isProcessing && (
                ...
                {uploadMode === UploadMode.Single && imagePreview && (
                 ...
                 {uploadMode === UploadMode.Batch && !downloadUrl && (
                    <div className="flex-grow flex flex-col items-center justify-center text-center p-4 bg-gray-800/50 rounded-lg">
                        ...
                        <p className="text-sm text-gray-500 mt-1">Generated tags will be available as a downloadable ZIP file here.</p>
                    </div>
                )}
                ...
                {output && (
                    ...
                    <button onClick={copyToClipboard} className="absolute top-2 right-2 p-2 rounded-md bg-gray-700 hover:bg-gray-600 transition-colors">
